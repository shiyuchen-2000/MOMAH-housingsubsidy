...
const I18N = {
  en:{
    ...
    audit_catAll:"All", audit_catPkg:"Decisions", audit_catFormula:"Formula", audit_catThreshold:"Threshold", audit_catRef:"Beneficiary", audit_catFair:"Fairness/Leakage", audit_catWhatif:"What-if",
...
function Allocation(){
  const {t,user,allocation,recalcAlloc,submitAlloc,actAlloc,setRoute,baseline}=useStore(); const {moneyFull}=useMoney();
  ...
  const annualBudget=BRD.phase3BudgetSAR/BRD.phase3Years;
  ...
  return (<div className="fade">
    ...
    <div className="dr-strip" style={{marginBottom:16}}>
      ...
      <div className={"mini-kpi"+(data.FG>=1.0?"":" alert-mini")}><div className="muted" style={{fontSize:11.5}}>Fairness Gap ≥ 1.0</div><div className="v" style={{color:data.FG>=1.0?"var(--primary)":"var(--amber)"}}>{data.FG>=1.0?"✅ "+data.FG.toFixed(2):"⚠️ "+data.FG.toFixed(2)}</div></div>
      <div className={"mini-kpi"+(data.HBR<=0.35?"":" alert-mini")}><div className="muted" style={{fontSize:11.5}}>HBR ≤ 35%</div><div className="v" style={{color:data.HBR<=0.35?"var(--primary)":"var(--amber)"}}>{data.HBR<=0.35?"✅ "+pct1(data.HBR):"⚠️ "+pct1(data.HBR)}</div></div>
...
function ForecastFairness(){
  const {t,user,leaks,leakAct,baseline}=useStore(); const {money}=useMoney();
  ...
  return (<div className="fade">
    ...
    <Section title={<span className="sect-right">{t("fairnessByRegion")}<InfoTip text={t("fml_fg")}/></span>}>
      <div className="banner" style={{marginBottom:0,background:"var(--green-50)",cursor:"pointer",fontWeight:600}} onClick={()=>setRoute("fairness")}>
        ⚖ {t("fairness_sub")} — <span style={{color:"var(--green-dark)",textDecoration:"underline"}}>{t("nav_fairness")}</span>
      </div>
    </Section>
    <Section title={t("leakage")} sub={t("leak_routeHint")}>
      <div className="banner" style={{marginBottom:0,background:"var(--amber-50)",cursor:"pointer",fontWeight:600}} onClick={()=>setRoute("fairness")}>
        ⚖ {t("leak_cases")}: 188 · {t("nav_fairness")} → <span style={{color:"var(--green-dark)",textDecoration:"underline"}}>{t("nav_fairness")}</span>
      </div>
    </Section>
...
/* ---- What-if — centerpiece ---- */
function WhatIf(){
  const {t,setRoute,addPackage,user,formulaParams,formulaVersion,setFormulaVersion,baseline}=useStore(); const {money}=useMoney();
...
/* ---- Owner home ---- */
function OwnerHome(){
  const {t,packages,budget,setRoute,baseline}=useStore(); const {money}=useMoney();
...
function AuditTrailPage(){
  ...
  const CATS=[["all","audit_catAll"],["pkg","audit_catPkg"],["formula","audit_catFormula"],["threshold","audit_catThreshold"],["ref","audit_catRef"],["fair","audit_catFair"],["whatif","audit_catWhatif"]];
  ...
  return (<div className="fade">
    ...
    <div className="card pad" style={{padding:"8px 16px"}}>
      <table className="tbl" style={{fontSize:12.5}}><thead><tr>
        <th style={{width:140}}>{t("set_g")}</th><th>{t("set_param")}</th><th style={{width:70}}>{t("set_affects")}</th><th style={{width:100}}></th><th style={{width:80,textAlign:"end"}}>{t("set_val")}</th><th style={{width:110}}></th>
      </tr></thead><tbody>
        {SETTINGS_GROUPS.flatMap((grp,gi)=>grp.items.map((it,ii)=>(
          <tr key={it.k}>
            ...
            <td className="right-num mono" style={{fontWeight:700,fontSize:14}}>{vals[it.k]}{it.unit}</td>
            <td><input className="input mono" style={{width:100,textAlign:"end"}} type="number" value={vals[it.k]} disabled={!editable}
              onChange={e=>{setVals({...vals,[it.k]:e.target.value}); setSaved(false);}}/></td>
          </tr>
        )))}
      </tbody></table>
    </div>
...
function App(){
  ...
  const store={ t,lang,setLang,currency,setCurrency,user,setUser,route,setRoute,packages,audit,addPackage,actOnPackage,reset,allocation,recalcAlloc,submitAlloc,actAlloc,leaks,leakAct,budget,saveBudget,formulaParams,setFormulaParams,formulaVersion,setFormulaVersion,baseline,pushAudit };
  ...
  if(user==="analyst"){
    ...
    page = route==="data"?<DataReadiness/> : route==="alloc"?<Allocation/> : route==="approvals"?<DecisionPackages filter={p=>p.status!=="draft"}/> : route==="referrals"?<BeneficiaryTracking/> : route==="fairness"?<FairnessLeakage/> : route==="forecast"?<ForecastFairness/>
